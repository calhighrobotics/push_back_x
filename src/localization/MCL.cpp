...
namespace MCL {
    ...
    // How much to correct the odometry towards the MCL estimate per tick.
    // 0.05 means it corrects 5% of the error every 30ms.
    constexpr float POSE_FILTER_GAIN = 0.05f; 
    ...
    // Reject any sensor reading beyond this.
    constexpr float MAX_SENSOR_READING = 65.0f;  // inches
    ...
    void MonteCarlo() {
        ...
        while (true) {
            ...
            if (std::abs(dX_global) > 0.001 ||
                ...
                // ── Filter & Update Odometry Belief ────────────────────
                // 1. Find the error between MCL and current odometry
                double err_x = global_X - currOdom.x;
                double err_y = global_Y - currOdom.y;

                // 2. Scale the correction by our filter gain AND our confidence.
                double applied_gain = POSE_FILTER_GAIN * global_Confidence;
                double corr_x = err_x * applied_gain;
                double corr_y = err_y * applied_gain;

                double new_odom_x = currOdom.x + corr_x;
                double new_odom_y = currOdom.y + corr_y;

                // 3. Update the chassis pose (LemLib)
                // Using an explicit lemlib::Pose constructor is safer against compiler warnings
                lemlib::Pose updatedPose(new_odom_x, new_odom_y, currOdom.theta);
                chassis.setPose(updatedPose);

                // 4. CRITICAL: Ensure the next motion update doesn't see 
                //    this programmatic correction as physical movement.
                currOdom = updatedPose;
            }
