...
void Chassis::moveToPoseRamsete(float targetX, float targetY, float targetTheta, int timeout_msec, const VelocityControllerConfig &config, MoveToPoseParams params, bool async) {
    ...
    // 1. Standard LemLib Async Boilerplate
    if (async) {
        pros::Task task([=, this]() {
            // Recursively call the function synchronously inside the new task
            moveToPoseRamsete(targetX, targetY, targetTheta, timeout_msec, config, params, false);
        });
        pros::delay(10); // Crucial: Gives the PROS scheduler time to start the task and acquire the mutex
        return;          // Exit the main thread immediately
    }

    // 2. Synchronous logic starts here (running inside the task if async == true)
    this->requestMotionStart(); 
    this->distTraveled = 0; 

    // Initialize your PID and controllers
    ...
    while ((pros::millis() - startTime < static_cast<uint32_t>(timeout_msec)) && (!lateralExit.getExit() || !longitudinalExit.getExit())) {
        ...
        // Listen for LemLib's internal cancellation flag (e.g., if a new motion is called)
        if (this->motionQueued) {
            break;
        }
    ...
    // Release the state machine so the next motion can begin
    this->motionRunning = false;
}
